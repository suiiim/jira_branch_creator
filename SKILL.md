...
description: >
  Jira 이슈 관리 및 GitLab 브랜치 자동 생성 CLI + 시스템 트레이 도구.
  이슈 조회/생성, 상태 전환, 브랜치 자동 생성을 하나의 워크플로우로 제공합니다.
  시스템 트레이에서 백그라운드 폴링 감시 모드를 지원합니다.
  "jira 브랜치", "이슈에서 브랜치", "branch from jira", "create branch",
  "이슈 생성", "상태 전환" 키워드에 반응합니다.
compatibility: Python 3.12+, requests, pystray, Pillow. Windows/macOS/Linux.
...
Jira 이슈를 관리하고 GitLab 브랜치를 자동 생성하는 CLI + 시스템 트레이 도구입니다.
...
| **Observer (pystray)** | `tray.py` | 시스템 트레이에서 폴링 감시 + 상태 알림 |
...
│   ├── config.py                     # 환경변수 → frozen dataclass 설정 (TrayConfig 포함)
│   ├── exceptions.py                 # 예외 계층 (JiraBranchCreatorError 루트)
│   ├── tray.py                       # 시스템 트레이 앱 (pystray 기반 백그라운드 감시)
│   ├── facades/
...
```
CLI (main.py)                          Tray (tray.py)
  → WorkflowFacade (facades/)            → WorkflowFacade (facades/)
    → JiraService (services/)              → JiraService (services/)
    → GitLabService (services/)             → GitLabService (services/)
    → branch_naming (utils/)               → branch_naming (utils/)
  → 결과 출력                              → 시스템 알림 (notify)
```

CLI와 트레이 모드 모두 동일한 WorkflowFacade를 사용합니다.

### 데이터 흐름

- 모든 모델(`JiraIssue`, `BranchInfo` 등)은 `frozen=True` dataclass로 불변
- API 응답은 `from_api_response()` 팩토리 메서드로 모델 변환
- 설정은 `AppConfig` → `JiraConfig` / `GitLabConfig` / `BranchNamingConfig` / `TrayConfig`로 계층 분리
...
- `TRAY_POLL_INTERVAL` (기본: `30`, 초 단위)
- `TRAY_AUTOSTART` (기본: `false`, 로그인 시 자동 시작)
- `TRAY_NOTIFY` (기본: `true`, 브랜치 생성 시 시스템 알림)
...
```bash
# 트레이 앱 실행 (백그라운드에서 Jira 폴링 + 브랜치 자동 생성)
jira-branch-tray
...
트레이 앱 기능:
- 시스템 트레이에서 동작 상태 확인 (아이콘 색상)
- 우클릭 메뉴: 시작/일시정지/종료, 폴링 간격 조정
- 새 브랜치 생성 시 시스템 알림 (Windows toast notification)
- 로그인 시 자동 시작 등록/해제 (시작 프로그램 폴더)
...
- `tray.py`에서 메뉴 항목, 폴링 로직, 알림 메시지 수정
- `TrayConfig`에 새 설정값 추가 시 `config.py`도 함께 수정
- pystray의 `icon.notify()`로 시스템 알림, `icon.update_menu()`로 메뉴 동적 갱신
